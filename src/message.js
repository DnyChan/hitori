...
const STATUS_CACHE_FILE = './database/status_cache.json';
function _bufReplacer(k, v) {
  if (Buffer.isBuffer(v)) return { __b64: v.toString('base64') };
  if (v && typeof v === 'object' && v.type === 'Buffer' && Array.isArray(v.data))
    return { __b64: Buffer.from(v.data).toString('base64') };
  return v;
}
function _bufReviver(k, v) {
...
function loadStatusCache() {
  try {
    if (!fs.existsSync(STATUS_CACHE_FILE)) return {};
    return JSON.parse(fs.readFileSync(STATUS_CACHE_FILE, 'utf8'), _bufReviver);
  } catch(e) { console.log('[STATUS CACHE] Load error:', e.message); return {}; }
}
let _saveCacheTimer = null;
function saveStatusCache() {
  clearTimeout(_saveCacheTimer);
  _saveCacheTimer = setTimeout(() => {
    try { fs.writeFileSync(STATUS_CACHE_FILE, JSON.stringify(global.statusCache || {}, _bufReplacer)); }
    catch(e) { console.log('[STATUS CACHE] Save error:', e.message); }
  }, 2000);
}
...
async function MessagesUpsert(naze, message, store) {
	try {
		...
		// Story Cache — simpan semua story masuk untuk fitur .sw
		if (msg.key.remoteJid === 'status@broadcast' && msg.key.participant && msg.message) {
			const storyType = getContentType(msg.message);
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage|conversation)/i.test(storyType)) {
				if (!global.statusCache) global.statusCache = {};
				let sender = msg.key.participant;
				// Resolve LID (@lid) ke nomor HP biasa (@s.whatsapp.net)
				if (sender && sender.endsWith('@lid') && typeof naze.findJidByLid === 'function') {
					const resolved = naze.findJidByLid(sender, store);
					if (resolved) sender = resolved.includes('@') ? resolved : resolved + '@s.whatsapp.net';
				}
				if (!global.statusCache[sender]) global.statusCache[sender] = [];
				// Hindari duplikat
				const alreadyExist = global.statusCache[sender].some(s => s.id === msg.key.id);
				if (!alreadyExist) {
					global.statusCache[sender].unshift({
						id: msg.key.id,
						rawMsg: msg,
						type: storyType,
						timestamp: Date.now(),
					...
					// Max 15 story per kontak
					if (global.statusCache[sender].length > 15) global.statusCache[sender].pop();
					saveStatusCache();
					console.log('[STORY CACHE] +' + sender.split('@')[0], '|', storyType, '| total:', global.statusCache[sender].length);
				}
			}
		}
	} catch (e) {
...
async function Solving(naze, store) {
	global.statusCache = loadStatusCache();
	...
	naze.sendAlbumMessage = async (jid, content = {}, options = {}) => {
		...
		if (album.length < 1) throw new RangeError("Minimum 1 media");
