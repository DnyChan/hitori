...
const question = (text) => new Promise((resolve) => {
	if (rl) return rl.question(text, resolve);
	console.log(chalk.yellow('[DEPLOY] Non-interactive environment - no stdin. Waiting for BOT_NUMBER or existing session...'))
	resolve('');
});
...
// HTTP Keep-Alive Server (mencegah Replit sleep & memastikan bot selalu aktif)
const PORT = process.env.PORT || 3000;
const keepAliveServer = http.createServer((req, res) => {
	res.writeHead(200, { 'Content-Type': 'text/plain' });
	res.end(req.url === '/ping' ? 'pong' : 'Bot WhatsApp is running!');
});
keepAliveServer.listen(PORT, () => {
	console.log(chalk.greenBright('[KEEP-ALIVE] HTTP server aktif di port ' + PORT));
});

async function startNazeBot() {
	if (_isReconnecting) return;
	_isReconnecting = true;
	setTimeout(() => { _isReconnecting = false; }, 10000);
	try {
	...
	naze.ev.on('connection.update', async (update) => {
		...
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.connectionLost) {
				...
				console.log('[SESSION] Session diambil instance lain. Dev bot idle — restart workflow jika perlu.');
	...
	if (!global._keepAlive) {
		global._keepAlive = setInterval(async () => {
			try {
				const selfUrl = process.env.REPLIT_DEV_DOMAIN
					? 'https://' + process.env.REPLIT_DEV_DOMAIN + '/ping'
					: 'http://localhost:3000/ping';
				await axios.get(selfUrl, { timeout: 5000 }).catch(() => {});
			} catch (e) {}
		}, 4 * 60 * 1000);
	}
